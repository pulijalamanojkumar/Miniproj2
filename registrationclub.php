 ...
 $con = new mysqli("localhost","root","","registrations");
	...
	if($_SESSION['ID']==4){
		...
		$select="select email from edc where email=? limit 1";
 $insert= "insert into edc(fullname,email,phone) values (?,?,?)";
...
$con->close(); 
	...
	if($_SESSION['ID']==3){
		...
		$select="select email from street where email=? limit 1";
 $insert= "insert into street(fullname,email,phone) values (?,?,?)";
 $stmt=$con->prepare($select);
 $stmt->bind_param("s",$email);
 $stmt->execute();
 $stmt->bind_result($email);
...
$con->close(); 
	...
	if($_SESSION['ID']==2){
		...
		$select="select email from mun where email=? limit 1";
 $insert= "insert into mun(fullname,email,phone) values (?,?,?)";
 $stmt=$con->prepare($select);
 $stmt->bind_param("s",$email);
 $stmt->execute();
 $stmt->bind_result($email);
...
$con->close(); 
	...
	if($_SESSION['ID']==1){
		...
		$select="select email from nss where email=? limit 1";
 $insert= "insert into nss(fullname,email,phone) values (?,?,?)";
 $stmt=$con->prepare($select);
 $stmt->bind_param("s",$email);
 $stmt->execute();
 $stmt->bind_result($email);
