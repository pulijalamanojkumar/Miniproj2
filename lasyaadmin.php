...
if(isset($_POST['imgupload'])){
 ...
 $sql = "insert into lasyaimg(image) values ('$file')";
...
$sql="select fullname,email,phone from lasya";
...
if(isset($_POST['uploadach'])){
 ...
 $file=$_FILES['image']['name'];
 $sql = "insert into lasya(description,image) values ('$desc','$file')";
 $res=mysqli_query($con,$sql);
if($res){
move_uploaded_file($_FILES['image']['tmp_name'],"$file");
echo "<script language='javascript'>";
...
if(isset($_POST['uploadbox'])&&($_POST['desc']!=NULL)){
	 ...
	 $sql="insert into lasya(description,links) values('$desc','$links')";
	 $stmt=$con->prepare($sql);
	 $stmt->execute();
	 echo "<script language='javascript'>";
...
if(isset($_POST['deleteupdate'])){
	 ...
	 $sql="delete from lasya where id in (select min(id) from kreeda)";
	 $stmt=$con->prepare($sql);
	 $stmt->execute();
	 	 echo "<script language='javascript'>";
            echo "alert('DELETED')";
