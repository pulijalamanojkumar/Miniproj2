...
if(isset($_POST['imgupload'])){
 ...
 $sql = "insert into streetimg(image) values ('$file')";
...
if(isset($_POST['deleteimg'])){
	 ...
	 if(mysqli_connect_error()){
		...
		$sql="delete from streetimg where id in (select min(id) from streetimg)";
...
$sql="select fullname,email,phone from street";
...
<div class="updatebox" style="position:absolute;left:35%;">
<?php
if(isset($_POST['uploadach'])){
	$desc=$_POST['desc'];
 $con = mysqli_connect("localhost","root","","clubach");
 $file=$_FILES['image']['name'];
 $sql = "insert into street(description,image) values ('$desc','$file')";
 $res=mysqli_query($con,$sql);
if($res){
move_uploaded_file($_FILES['image']['tmp_name'],"$file");
echo "<script language='javascript'>";
...
if(isset($_POST['uploadbox'])&&($_POST['desc']!=NULL)){
	 ...
	 $sql="insert into street(description,links) values('$desc','$links')";
	 $stmt=$con->prepare($sql);
	 $stmt->execute();
	 	 echo "<script language='javascript'>";
...
if(isset($_POST['deleteupdate'])){
	 ...
	 $sql="delete from street where id in (select min(id) from street)";
	 $stmt=$con->prepare($sql);
	 $stmt->execute();
	 	 echo "<script language='javascript'>";
            echo "alert('DELETED')";
